...
import VideoBox from '../component/VideoBox.js';
export default class Video extends FreedomInterface {
    ...
    constructor(dataset) {
        ...
        if (!dataset && Object.entries(this.dataset).length == 0) {
            ...
            Video.customVideoCallback &&
            typeof Video.customVideoCallback == 'function'
        ) {
    ...
    createDefaultContent() {
        return new Promise((resolve) => {
            ...
            this.video.onload = () => {
                if (
                    (this.file.length != 0 &&
                        video.canPlayType(this.file.files[0].type) == '') ||
                    (this.dataset.name &&
            ...
            this.connectedAfterOnlyOneCallback = () => {
                ...
                wrap.replaceChildren(
                    ...[description, videoContanier].filter(
                        (e) => e != undefined,
                    ),
                );
    ...
    createDescription(video, videoContanier) {
        ...
        description.onclick = (event) => {
            if (description.dataset.open_status == '▼') {
                ...
                videoContanier.style.height =
                    window.getComputedStyle(video).height;
                setTimeout(() => {
