export default class ToolHandler {
    ...
    constructor(identity) {
        ...
        document.addEventListener('selectionchange', (event) => {
            ...
            if (
                !document.activeElement.classList.contains('free-will-editor')
            ) {
                return;
            }
            /**
            ...
            let findTarget = this.#connectedFriends.find(
                (e) =>
                    selection.containsNode(e, true) ||
                    selection.containsNode(e, false),
            );
            if (findTarget) {
    ...
    isLastTool(tool) {
        if (this.#identity.prototype.isPrototypeOf(tool)) {
            ...
            throw new Error(
                `tool is not my identity, this tool name is ${tool.constructor.name}. but my identity name is ${this.#identity.name}`,
            );
        }
    }
    ...
    set toolButton(toolButton) {
        if (
            !toolButton ||
            !toolButton.nodeType ||
            toolButton.nodeType != Node.ELEMENT_NODE
        ) {
