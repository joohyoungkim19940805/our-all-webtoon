...
export default class FreedomInterface extends HTMLElement {
     ...
     */
    static globalKeydownEventListener(
        element,
    ...
    static globalSelectionChangeEventListener(
        element,
        callback = ({ oldEvent, newEvent }) => {},
    ) {
        let oldEvent = undefined;
     ...
     * @param {Function} callback
     */
    static outClickElementListener(
        element,
        callback = ({ oldEvent, newEvent, isMouseOut = false }) => {},
    ) {
    ...
    static DeleteOption = class DeleteOption {
        ...
        static EMPTY_CONTENT_IS_DELETE = new this.#DeleteOptionEnum(
            'empty_content_is_delete',
        );
        static EMPTY_CONTENT_IS_NOT_DELETE = new this.#DeleteOptionEnum(
            'empty_content_is_not_delete',
        );
        value;
        static {
            Object.freeze(this);
    ...
    constructor(
        ...
        dataset,
        {
            deleteOption = FreedomInterface.DeleteOption
                .EMPTY_CONTENT_IS_DELETE,
        } = {},
    ) {
        super();
        ...
        FreedomInterface.globalSelectionChangeEventListener(
            this,
            ({ oldEvent, newEvent }) => {
                if (
                    this.#deleteOption !=
                        FreedomInterface.DeleteOption
                            .EMPTY_CONTENT_IS_NOT_DELETE &&
                    this.isToolEmpty()
                ) {
    ...
    connectedCallback() {
        if (!this.#isLoaded) {
            ...
            if (
                this.#deleteOption ==
                    FreedomInterface.DeleteOption.EMPTY_CONTENT_IS_DELETE &&
                (this.isToolEmpty() || this.childNodes.length == 0)
            ) {
                let thisLine = this.parentEditor.getLine(this);
                this.remove();
                if (thisLine) {
