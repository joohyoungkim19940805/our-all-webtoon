...
export default new (class Common {
    ...
    jsonToSaveElementDataset(data: object, element: HTMLElement) {
        if (!element) {
            ...
            throw new Error(
                `element is not element node type ::: ${element.nodeType}`,
            );
        }
        return new Promise((resolve) => {
            let underbarKeyNameObject = Object.entries(data).reduce(
                (total, [k, v]) => {
                    let key = (k.match(this.#keyRegx) || [])
                        .map((e) => e.toLowerCase())
                        .join('_');
                    total[key] = v;
                    return total;
    ...
    processingElementPosition(
        ...
        if (target instanceof DOMRect) {
            // this.isElement(target, HTMLElement)){
            rect = target;
        } else {
	...
	*/
    shortenBytes(byte: number) {
        ...
        const rankText =
            ((rank > 0 ? 'KMGTPEZY'[rank - 1] : '') || (rank >= 9 ? 'Y' : '')) +
            'B';
