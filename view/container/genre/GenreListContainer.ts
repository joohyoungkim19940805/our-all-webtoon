...
export const genreListContainer = (genreList :Genre[], genreItemType : GenreItemType) => {
	const ul = Object.assign(document.createElement('ul'),{
		className: styles["genre-list-container"]
	});
	return zip(genreList.map((e,i)=> {
		return genreItem(genreItemType, i, e)
	})).pipe(mergeMap(e=>e), toArray(), map(liList=>{
...
export const genreItem = (( {type} : GenreItemType, index:number, genre:Genre ) => {
	return input(
		{ 
			type, 
			data: { 
			...
			id: `genre-list-item_${index}`, name:'genre-list', hidden: true
		},
		{}
	).pipe(map(input=>{
