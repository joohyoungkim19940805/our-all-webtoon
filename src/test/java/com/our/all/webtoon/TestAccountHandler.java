...
//
    ...
    public void testGetUserById() {
        ...
        webTestClient.get().uri("/users/1").accept(MediaType.APPLICATION_JSON).exchange().expectStatus().isOk().expectBody(User.class).value(userResponse -> {
    ...
    public void testGetUsers() {
        ...
        webTestClient.get().uri("/users").exchange().expectStatus().isOk().expectBodyList(User.class).value(userResponse -> {
    ...
    public void testCreateUser() {
        ...
        webTestClient
            .post().uri("/users").contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).body(Mono.just(user), User.class).exchange().expectStatus().isOk().expectBody(User.class)
            .value(userResponse -> {
                Assertions.assertThat(userResponse.getId()).isEqualTo(1);
                Assertions.assertThat(userResponse.getName()).isEqualTo("ABC");
                Assertions.assertThat(userResponse.getEmail()).isEqualTo("abc@xyz.com");
