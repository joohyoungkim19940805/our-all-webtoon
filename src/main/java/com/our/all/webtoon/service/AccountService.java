...
public class AccountService {
    ...
    public Mono<Token> authenticate(Mono<AccountEntity> accountEntityMono, Optional<InetSocketAddress> optional) {
        return accountEntityMono.flatMap(accountInfo -> {
            return accountRepository.findByAccountName(accountInfo.getAccountName()).switchIfEmpty(Mono.error(new AccountException(Result._103))).flatMap(account -> {
                ...
                if (!account.getIsEnabled()) {
                    ...
                    AccountLogEntity accountLogEntity = AccountLogEntity.builder().accountId(account.getAccountName()).ip(optional.get().getAddress().getHostAddress()).build();
