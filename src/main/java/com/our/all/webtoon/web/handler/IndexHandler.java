...
public class IndexHandler {
    ...
    public Mono<ServerResponse> loginPage(ServerRequest request) {
        ...
        return ok().contentType( MediaType.parseMediaType( "text/html;charset=UTF-8" ) )
            .render(
                ...
                Map.of( "resourcesNameList", List.of( "loginPage" ) ) );
    ...
    public Mono<ServerResponse> create(ServerRequest request) {
        ...
        return accountService.createUser( request.bodyToMono( AccountEntity.class ), true )
            .flatMap(
                account -> ok().contentType( MediaType.APPLICATION_JSON )
    ...
    public Mono<ServerResponse> accountVerifyPage(ServerRequest request) {
		...
		return jwtVerifyHandler.getJwt( token ).flatMap( jws -> {
            ...
            if (! JwtIssuerType.ACCOUNT_VERIFY.name()
                .equals( String.valueOf( header.get( "jwtIssuerType" ) ) ) || expiration
                    .before( new Date() )) {
                return body.render( returnHtml );
            }
            return body
                .render( returnHtml, Map.of( "email", claims.getSubject(), "token", token ) );
        } )
    ...
    public Mono<ServerResponse> accountVerify(ServerRequest request) {
         ...
         * 
         * Jws<Claims> jws = jwtVerifyHandler.getJwt(token); Claims claims = jws.getBody();
         * JwsHeader<?> header = jws.getHeader(); Date expiration = claims.getExpiration();
         * 
         * if( ! JwtIssuerType.ACCOUNT_VERIFY.name().equals(
         * String.valueOf(header.get("jwtIssuerType")) )) { return ok()
         * .contentType(MediaType.parseMediaType("text/html;charset=UTF-8"))
         ...
         * 
         * return ok() .contentType(MediaType.parseMediaType("text/html;charset=UTF-8"))
         * .render("content/account/accountVerifyTemplate.html", Map.of( "email",
         * claims.getSubject(), "token", token )); }) .onErrorResume(e->{ return Mono.error(new
    ...
    public Mono<ServerResponse> loginProcessing(ServerRequest request) {
        ...
        return accountService
            ...
            .flatMap(
                token -> ok().cookie(
                    ResponseCookie.fromClientResponse( HttpHeaders.AUTHORIZATION, token.getToken() )
                        .httpOnly( false )
                        .sameSite( "Strict" )
                        .path( "/" )
                        .build() )
    ...
    public Mono<ServerResponse> logout(ServerRequest request) {
        ...
        return ok()
            .cookie(
                ResponseCookie.fromClientResponse( HttpHeaders.AUTHORIZATION, "" )
                    .httpOnly( false )
                    .sameSite( "Strict" )
                    .path( "/" )
                    .build() )
    ...
    public Mono<ServerResponse> forgotPassword(ServerRequest request) {
        ...
        return request.bodyToMono( AccountEntity.class )
            ...
            .doOnNext( account -> {
                ...
                /* Mono.just(e) .publishOn(Schedulers.boundedElastic())
                 * .subscribe(account->{
                 * mailService.sendForgotPasswordEmail(account); }); */
            } )
    ...
    public Mono<ServerResponse> changePassword(ServerRequest request) {
        ...
        return request.bodyToMono( ChangePasswordRequest.class )
            .flatMap(
                changePasswordRequest -> accountRepository
                    .findByEmail( changePasswordRequest.email() )
                    .switchIfEmpty( Mono.error( new AccountException( Result._1 ) ) )
                    .map( accountEntity -> {
                        accountEntity.setPassword(
                            accountService.passwordEncoder
                                .encode( changePasswordRequest.password() ) );
                        return accountEntity;
                    } ) )
            .flatMap( accountEntity -> accountRepository.save( accountEntity ) )
            .switchIfEmpty( Mono.error( new ForgotPasswordException( Result._108 ) ) )
     ...
     * request){ //Long workspaceId = Long.valueOf(request.pathVariable("workspaceId")); //Long
     * roomId = Long.valueOf(request.pathVariable("roomId")); return ok()
     * .contentType(MediaType.parseMediaType("text/html;charset=UTF-8"))
    ...
    public Mono<ServerResponse> createSubWindow(ServerRequest request) {
        ...
        return ok().contentType( MediaType.parseMediaType( "text/html;charset=UTF-8" ) )
            .render( "electron_web/" + pageName + ".html" );// , Map.of("workspaceId",
                                                            // workspaceId,
                                                            // "roomId", roomId));
