...
public class MainRouter implements MainRouterSwgger {
    ...
    public RouterFunction<ServerResponse> gnb(GnbHandler gnbHandler) {
        return route().nest(path("/gnb"),
                builder -> builder.nest(path("search"), searchPathBuilder -> searchPathBuilder
                        .GET("/my", accept(MediaType.APPLICATION_JSON), gnbHandler::my).build()))
                .build();
    }
}
