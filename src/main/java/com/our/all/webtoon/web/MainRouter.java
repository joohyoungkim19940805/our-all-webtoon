...
public class MainRouter implements MainRouterSwgger{
	...
	public RouterFunction<ServerResponse> index(IndexHandler indexHandler){
    	...
    	return route( GET("/"), req -> ServerResponse.temporaryRedirect(URI.create("/login-page")).build() )
				...
				.and(route( GET("/account-verify/{token}").and(accept(MediaType.TEXT_HTML)), indexHandler::accountVerifyPage ))
				...
				.and(route( GET("/change-password-page/{token}").and(accept(MediaType.TEXT_HTML)), indexHandler::changePasswordPage ))
				...
				.and(route( GET("/web/main").and(accept(MediaType.TEXT_HTML)), indexHandler::webMain))
				...
				.and(route( GET("/web/create-sub-window/{pageName}").and(accept(MediaType.TEXT_HTML)), indexHandler::createSubWindow ))
